...
# sort them from left to right, initialize a dictionary to map
...
# apply a tophat (whitehat) morphological operator to find light regions against 
# a dark background (i.e., the credit card numbers)
...
# apply a closing operation using the rectangular kernel to help cloes gaps in between 
# credit card number digits, then apply Otsu's thresholding method to binarize the image
...
for (i, c) in enumerate(cnts):
	# compute the bounding box of the contour, then use the bounding box coordinates 
    # to derive the aspect ratio
	(x, y, w, h) = cv2.boundingRect(c)
	ar = w / float(h)
	# since credit cards used a fixed size fonts with 4 groups of 4 digits, we can 
    # prune potential contours based on the aspect ratio
